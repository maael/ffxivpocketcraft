const { EXCLUDED_SERVERS, MONGO_URI, MONGO_DB } = require('dotenv-extended').load()
...
module.exports = async function start (dbs) {
  agenda.define('update market data', async (_, done) => {
    marketDebug('starting market job')
  ...
  await agenda.start()
  await agenda.every('1 day', 'update market data')
}
